...
def Recognition(subject):
    ...
    completed_label = "Attendance Marked"
    reset_keras()
    e = emb()
    fd = face()
    ...
    while ret:
            ...
            for i in range(len(det)):
                ...
                try:
                    if (int(attendance_count[person]) >= 30):
                        cv2.putText(
                            frame, completed_label, (k[0], k[1]), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
                        cv2.rectangle(
                            frame, (k[0], k[1]), (k[0]+k[2], k[1]+k[3]), (0, 255, 0), 3)
                    else:
                        cv2.putText(
                            frame, label, (k[0], k[1]), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
                        cv2.rectangle(
                            frame, (k[0], k[1]), (k[0]+k[2], k[1]+k[3]), (255, 0, 0), 3)
                except:
                    pass
        ...
        if (cv2.waitKey(1) & 0XFF == ord('q')):
            break
        if cv2.getWindowProperty('Say Cheese and Press "Q" to Quit', cv2.WND_PROP_VISIBLE) < 1:
            break
